...
Vagrant.configure(2) do |config|
  ...
  config.vm.synced_folder ".", "/var/www/app.local/public", :owner => "www-data", :group => "www-data"
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    cd /var/www/app.local/public
    sudo composer install
    sudo chmod +x /var/www/app.local/public/bin/cake
    sudo composer require --dev cakephp/debug_kit "~3.0"
    sudo bin/cake plugin load DebugKit
    sudo cp /var/www/app.local/public/config/nginx/app.local /etc/nginx/sites-available/app.local
    sudo mkdir -p /var/www/app.local/log
